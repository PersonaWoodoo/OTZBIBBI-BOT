...
def deposit_keyboard(lang='ru'):
    buttons = []
    # Кнопки для быстрого пополнения: 1, 2, 5, 10, 15
    ...
    # Кнопка для 15+ (ручное пополнение через админа)
...
# ========== ПОПОЛНЕНИЕ ЧЕРЕЗ STARS (ОФИЦИАЛЬНО) ==========
...
async def process_deposit(callback: CallbackQuery):
    ...
    if data[1] == "manual":
        # Ручное пополнение от 15⭐ через админа
        await callback.message.delete()
        await callback.message.answer(
            get_text(lang, 'deposit_manual', stars="15+", support=SUPPORT_USERNAME, request_id=""),
            reply_markup=back_keyboard(lang)
        )
        await callback.answer()
        # Здесь можно добавить состояние для ввода суммы
        return
    
    stars = int(data[1])
    get_amount = stars * STAR_TO_GET
    
    # Создаём счёт для оплаты через Telegram Stars
    prices = [LabeledPrice(label="GET Coins", amount=stars * 100)]  # 1 Star = 100 копеек
...
async def successful_payment(message: Message):
    ...
    # Получаем сумму в Stars (total_amount в копейках, 1 Star = 100 копеек)
...
async def create_task_start(message: Message, state: FSMContext):
    ...
    lang = user[2]
    if user[3] < 5000:
        await message.answer(get_text(lang, 'not_enough', balance=user[3]))
        return
    
    await message.answer(
        "📝 Укажите @username для отзыва (с @):" if lang == 'ru' else "📝 Enter @username for review (with @):",
        reply_markup=back_keyboard(lang)
    )
...
async def create_task_username(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Укажите тематику (без 18+ и скама):" if lang == 'ru' else "Enter theme (no 18+ or scam):",
        reply_markup=back_keyboard(lang)
    )
...
async def create_task_theme(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Количество отзывов (число):" if lang == 'ru' else "Number of reviews (number):",
        reply_markup=back_keyboard(lang)
    )
...
async def create_task_type(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Напишите пример отзыва:" if lang == 'ru' else "Write an example review:",
        reply_markup=back_keyboard(lang)
    )
