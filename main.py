...
GET_TO_STARS = {
    ...
    150000: {"name": "🌹  25⭐", "stars": 25},
    300000: {"name": "🚀 50⭐", "stars": 50},
    600000: {"name": "💍  100⭐", "stars": 100},
}
...
async def verify_user(user_id, device_id):
    """Проверка на твинка и бота (капча)"""
    user = get_user(user_id)
    if not user:
        return False
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if not await check_subscription(user_id):
        ...
        await message.answer(
            get_text(user[2], 'subscribe', channel=CHANNEL_ID),
...
async def set_language(callback: CallbackQuery, state: FSMContext):
    ...
    device_id = str(callback.from_user.id)  # В реальности нужно получать реальный device_id
    
    # Создаем пользователя
    user = create_user(user_id, username, device_id, language=lang)
    if not user:
    ...
    if not await check_subscription(user_id):
        ...
        await callback.message.edit_text(
            get_text(lang, 'subscribe', channel=CHANNEL_ID),
            reply_markup=keyboard
        )
        return
...
async def show_referrals(message: Message):
    ...
    await message.answer(
        get_text(lang, 'referrals', count=len(refs), code=user[3], bot=BOT_USERNAME)
    )
...
async def auto_deposit(callback: CallbackQuery):
    ...
    # Создаем ссылку на оплату через Stars
    # В реальности нужно использовать Telegram Stars API
    ...
    # Начисляем GET (в реальности после подтверждения оплаты)
...
async def give_contest_bonuses_execute(message: Message, state: FSMContext):
    ...
    bonuses = [contest[3], contest[3] // 2, contest[3] // 3]  # 1-е, 2-е, 3-е место
...
async def add_channel(message: Message, state: FSMContext):
    ...
    global CHANNEL_ID
    await message.answer(
        "Введите @канал:" if lang == 'ru' else "Enter @channel:",
        reply_markup=back_keyboard(lang)
    )
...
async def add_channel_save(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(
        f"✅ Канал {CHANNEL_ID} добавлен" if lang == 'ru' else f"✅ Channel {CHANNEL_ID} added",
        reply_markup=admin_keyboard(lang)
    )
